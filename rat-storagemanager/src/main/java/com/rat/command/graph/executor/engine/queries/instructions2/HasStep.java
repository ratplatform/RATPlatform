...
public class HasStep implements IInstruction{
	...
	public IInstructionResult execute(IInstructionInvoker invoker, ICommandNodeVisitable nodeCaller) throws Exception {
		try{
			...
			Step step = new Step();
			step.paramName = paramName;
			step.paramValue = paramValue;
			step.stepType = Counter.StepType.Has;
...
//			Counter.getInstance().inc();
			...
			//System.out.println("HasStep size: " + pipe.size());
